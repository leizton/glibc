...
static int __pthread_mutex_unlock_full(pthread_mutex_t* mutex, int decr) __attribute_noinline__;
...
int __pthread_mutex_unlock_usercnt(pthread_mutex_t* mutex, int decr) {
  ...
  normal:
    ...
    if (--mutex->__data.__count != 0)
      ...
      return 0;
    goto normal;
  } else if (PTHREAD_MUTEX_TYPE(mutex) == PTHREAD_MUTEX_ADAPTIVE_NP)
    goto normal;
  else {
    /* Error checking mutex.  */
    assert(type == PTHREAD_MUTEX_ERRORCHECK_NP);
    if (mutex->__data.__owner != THREAD_GETMEM(THREAD_SELF, tid) || !lll_islocked(mutex->__data.__lock))
      return EPERM;
    goto normal;
  }
}
...
__pthread_mutex_unlock_full(pthread_mutex_t* mutex, int decr) {
  ...
  case PTHREAD_MUTEX_ROBUST_RECURSIVE_NP:
    /* Recursive mutex.  */
    if ((mutex->__data.__lock & FUTEX_TID_MASK) == THREAD_GETMEM(THREAD_SELF, tid) && __builtin_expect(mutex->__data.__owner == PTHREAD_MUTEX_INCONSISTENT, 0)) {
      if (--mutex->__data.__count != 0)
        /* We still hold the mutex.  */
  ...
  case PTHREAD_MUTEX_ROBUST_ADAPTIVE_NP:
    if ((mutex->__data.__lock & FUTEX_TID_MASK) != THREAD_GETMEM(THREAD_SELF, tid) || !lll_islocked(mutex->__data.__lock))
      return EPERM;

    /* If the previous owner died and the caller did not succeed in
 making the state consistent, mark the mutex as unrecoverable
 and make all waiters.  */
    if (__builtin_expect(mutex->__data.__owner == PTHREAD_MUTEX_INCONSISTENT, 0))
    notrecoverable:
 ...
 FUTEX_WAITERS set previously, then wake any waiters.
    ...
    if (__glibc_unlikely((atomic_exchange_rel(&mutex->__data.__lock, 0) & FUTEX_WAITERS) != 0))
 ...
 in sysdeps/nptl/bits/thread-shared-types.h.  */
    if ((atomic_load_relaxed(&(mutex->__data.__kind)) & PTHREAD_MUTEX_ROBUST_NORMAL_NP) != 0 && __builtin_expect(mutex->__data.__owner == PTHREAD_MUTEX_INCONSISTENT, 0))
    pi_notrecoverable:
      newowner = PTHREAD_MUTEX_NOTRECOVERABLE;

    /* See concurrency notes regarding __kind in struct __pthread_mutex_s
 in sysdeps/nptl/bits/thread-shared-types.h.  */
    if ((atomic_load_relaxed(&(mutex->__data.__kind)) & PTHREAD_MUTEX_ROBUST_NORMAL_NP) != 0) {
    continue_pi_robust:
      /* Remove mutex from the list.
 ...
 lll_unlock).  */
    /* See concurrency notes regarding __kind in struct __pthread_mutex_s
 in sysdeps/nptl/bits/thread-shared-types.h.  */
    int robust = atomic_load_relaxed(&(mutex->__data.__kind)) & PTHREAD_MUTEX_ROBUST_NORMAL_NP;
    private = (robust
 ...
 synchronize with acquire MO in lock acquisitions.  */
    ...
    do {
      if (((l & FUTEX_WAITERS) != 0) || (l != THREAD_GETMEM(THREAD_SELF, tid))) {
 ...
 PTHREAD_MUTEX_ROBUST_NORMAL_NP.  */
  ...
  case PTHREAD_MUTEX_PP_ERRORCHECK_NP:
    /* Error checking mutex.  */
    if (mutex->__data.__owner != THREAD_GETMEM(THREAD_SELF, tid) || (mutex->__data.__lock & ~PTHREAD_MUTEX_PRIO_CEILING_MASK) == 0)
      return EPERM;
    /* FALLTHROUGH */
...
weak_alias(__pthread_mutex_unlock, pthread_mutex_unlock) hidden_def(__pthread_mutex_unlock)
