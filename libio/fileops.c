  ...
  if (result != NULL) {
    ...
    if (cs != NULL) {
     ...
     for these.  */
      ...
      cc->__cd_out.__cd.__data[0].__flags = __GCONV_IS_LAST | __GCONV_TRANSLIT;
...
libc_hidden_ver(_IO_new_file_fopen, _IO_file_fopen)
  ...
  if (_IO_SEEKOFF(fp, (off64_t)0, _IO_seek_cur, _IOS_INPUT | _IOS_OUTPUT) == _IO_pos_BAD && errno != ESPIPE)
    return NULL;
...
static int mmap_remap_check(FILE* fp) {
  ...
  if (_IO_SYSSTAT(fp, &st) == 0 && S_ISREG(st.st_mode) && st.st_size != 0
...
#define ROUNDED(x) (((x) + pagesize - 1) & ~(pagesize - 1))
    if (ROUNDED(st.st_size) < ROUNDED(fp->_IO_buf_end - fp->_IO_buf_base)) {
      ...
      (void)__munmap(fp->_IO_buf_base + ROUNDED(st.st_size),
                     ROUNDED(fp->_IO_buf_end - fp->_IO_buf_base) - ROUNDED(st.st_size));
      fp->_IO_buf_end = fp->_IO_buf_base + st.st_size;
    } else if (ROUNDED(st.st_size) > ROUNDED(fp->_IO_buf_end - fp->_IO_buf_base)) {
      /* The file added some pages.  We need to remap it.  */
      void* p;
#if _G_HAVE_MREMAP
      p = __mremap(fp->_IO_buf_base, ROUNDED(fp->_IO_buf_end - fp->_IO_buf_base),
...
_IO_new_file_seekoff(FILE* fp, off64_t offset, int dir, int mode) {
  ...
  int must_be_exact = (fp->_IO_read_base == fp->_IO_read_end && fp->_IO_write_base == fp->_IO_write_ptr);

  bool was_writing = (fp->_IO_write_ptr > fp->_IO_write_base || _IO_in_put_mode(fp));
  ...
  if (fp->_offset != _IO_pos_BAD && fp->_IO_read_base != NULL && !_IO_in_backup(fp)) {
    off64_t start_offset = (fp->_offset - (fp->_IO_read_end - fp->_IO_buf_base));
