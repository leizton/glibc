...
libc_hidden_proto(__lll_lock_wait_private) extern void __lll_lock_wait(int* futex, int private);
...
#define lll_cond_lock(futex, private)         \
  {                                           \
    int* __futex = &(futex);                  \
    if (atomic_exchange_acq(__futex, 2) != 0) \
      __lll_lock_wait(__futex, private);      \
  }

    //
    extern void __lll_lock_wake_private(int* futex);
libc_hidden_proto(__lll_lock_wake_private) extern void __lll_lock_wake(int* futex, int private);
